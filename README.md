...
- 数据源：主源东方财富（前复权），东财不可达时**自动切换新浪**（不复权），并有本地 CSV 缓存
...
├── data_fetcher.py    # AkShare 数据获取 + 多源自动降级 + CSV 缓存
...
`config.py` 里还能改：初始资金、佣金费率（默认万 1）、单笔最低佣金、滑点、缓存天数等。

## 数据与网络说明

- 首次使用某只 ETF 需要联网拉取，之后优先读本地缓存（默认 1 天内不重复请求）。
- 东财源被网络屏蔽/限流时程序会打印 `[提示]` 并自动使用新浪源（不复权，对 ETF 分红影响很小）。
